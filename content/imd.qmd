...
IMD is very useful for categorising the area a person lives in for deprivation. 
Deprivation is by country and do not include the other nations.
Deciles are the most commonly used way of referring to IMD and are taken from the scores which are ordered and then cut into 10.
For deciles the 1 is the most deprived area and 10 is the least deprived.
...
Postcode lengths vary in original data depending on whether one or two spaces are used between the parts.
Consequently, it is always better when joining by postcodes to remove the spaces altogether.
...
Partial postcodes are sometimes provided to protect the data and may be the first part (before spaces) and 1 or 2 characters from the second part.
This will not give a sufficiently reliable IMD score.
...
This code will not run and is dependent on the naming conventions of the SQL server. 
The column names of LSOA11 and `LSOAcode2011` will have come from the data sources. 
The column `PostCode_space` has been added to the original data.
...
Packages like [{NHSRpopulations}](https://github.com/nhs-r-community/NHSRpopulation) have functions to do this and more information in the [vignette](https://nhs-r-community.github.io/NHSRpopulation/articles/calc-imd-ranks-within-la.html).
...
# [TODO] find way to join to postcode table for LSOA code
